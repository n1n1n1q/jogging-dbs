...
class EventOperationsTest(BaseJoggingTest):
    ...
    def test_create_event(self):
        ...
        before_events = self._check_table_state("joggingevent", "EventName LIKE 'Test Event%'")
        ...
        after_events = self._check_table_state("joggingevent", "EventName LIKE 'Test Event%'")
        
        self.assertEqual(len(before_events), 0, "There should be no test events before the operation")
        self.assertEqual(len(after_events), 1, "There should be one test event after the operation")
    ...
    def test_update_event(self):
        ...
        before_event = self._check_table_state("joggingevent", "EventID = %s", (event_id,))[0]
        ...
        after_event = self._check_table_state("joggingevent", "EventID = %s", (event_id,))[0]
    ...
    def test_delete_event(self):
        ...
        before_state = self._check_table_state("joggingevent", "EventID = %s", (event_id,))
        
        with get_db_cursor(commit=False, connection=self.connection) as cursor:
            cursor.execute("DELETE FROM joggingevent WHERE EventID = %s", (event_id,))
        
        after_state = self._check_table_state("joggingevent", "EventID = %s", (event_id,))
    ...
    def test_event_registration(self):
        ...
        self.assertEqual(len(before_registrations), 0, "Should not be registered before operation")
        self.assertEqual(len(mid_registrations), 1, "Should be registered after operation")
        ...
        self.assertEqual(len(after_registrations), 0, "Should not be registered after unregistration")
