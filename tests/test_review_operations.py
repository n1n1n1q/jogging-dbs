...
class ReviewOperationsTest(BaseJoggingTest):
    ...
    def test_create_route_review(self):
        ...
        self.assertEqual(len(before_reviews), 0, "There should be no reviews before the operation")
        self.assertEqual(len(after_reviews), 1, "There should be one review after the operation")
    ...
    def test_update_route_review(self):
        ...
        with get_db_cursor(commit=False, connection=self.connection) as cursor:
            cursor.execute(
                "INSERT INTO routereview (RouteID, JoggerEmail, Rating, Comment) VALUES (%s, %s, %s, %s)",
                (self.route_id, self.test_jogger_email, initial_rating, initial_comment)
            )
            review_id = cursor.lastrowid
        
        before_review = self._check_table_state("routereview", "ReviewID = %s", (review_id,))[0]
        ...
        after_review = self._check_table_state("routereview", "ReviewID = %s", (review_id,))[0]
    ...
    def test_delete_route_review(self):
        ...
        before_state = self._check_table_state("routereview", "ReviewID = %s", (review_id,))
        
        with get_db_cursor(commit=False, connection=self.connection) as cursor:
            cursor.execute("DELETE FROM routereview WHERE ReviewID = %s", (review_id,))
        
        after_state = self._check_table_state("routereview", "ReviewID = %s", (review_id,))
    ...
    def test_update_event_review(self):
        ...
        with get_db_cursor(commit=False, connection=self.connection) as cursor:
            cursor.execute(
                "INSERT INTO eventreview (EventID, JoggerEmail, Rating, Comment) VALUES (%s, %s, %s, %s)",
                (self.event_id, self.test_jogger_email, initial_rating, initial_comment)
            )
            review_id = cursor.lastrowid
        
        before_review = self._check_table_state("eventreview", "ReviewID = %s", (review_id,))[0]
        ...
        after_review = self._check_table_state("eventreview", "ReviewID = %s", (review_id,))[0]
    ...
    def test_delete_event_review(self):
        ...
        before_state = self._check_table_state("eventreview", "ReviewID = %s", (review_id,))
        
        with get_db_cursor(commit=False, connection=self.connection) as cursor:
            cursor.execute("DELETE FROM eventreview WHERE ReviewID = %s", (review_id,))
        
        after_state = self._check_table_state("eventreview", "ReviewID = %s", (review_id,))
    ...
    def test_get_average_ratings(self):
        ...
        for i, rating in enumerate(route_ratings):
            ...
            with get_db_cursor(commit=False, connection=self.connection) as cursor:
                cursor.execute(
                    "INSERT INTO routereview (RouteID, JoggerEmail, Rating, Comment) VALUES (%s, %s, %s, %s)",
                    (self.route_id, jogger_email, rating, f"Review {i} with rating {rating}")
                )
        ...
        for i, rating in enumerate(event_ratings):
            ...
            with get_db_cursor(commit=False, connection=self.connection) as cursor:
                cursor.execute(
                    "INSERT INTO eventreview (EventID, JoggerEmail, Rating, Comment) VALUES (%s, %s, %s, %s)",
                    (self.event_id, jogger_email, rating, f"Event review {i} with rating {rating}")
                )
