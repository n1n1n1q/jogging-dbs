...
class SessionOperationsTest(BaseJoggingTest):
    ...
    def test_create_session(self):
        ...
        before_sessions = self._check_table_state(
            "jogging", "JoggerEmail = %s", (self.test_jogger_email,)
        )
        ...
        self.assertEqual(
            len(before_sessions), 0, "There should be no sessions before the operation"
        )
        self.assertEqual(
            len(after_sessions), 1, "There should be one session after the operation"
        )
    ...
    def test_update_session(self):
        ...
        before_session = self._check_table_state(
            "jogging", "SessionID = %s", (session_id,)
        )[0]
        ...
        after_session = self._check_table_state(
            "jogging", "SessionID = %s", (session_id,)
        )[0]
    ...
    def test_session_event_association(self):
        ...
        self.assertEqual(
            len(before_associations), 0, "Association should not exist before operation"
        )
        self.assertEqual(
            len(mid_associations), 1, "Association should exist after operation"
        )
        ...
        self.assertEqual(
            len(after_associations), 0, "Association should not exist after removal"
        )
