...
class RouteOperationsTest(BaseJoggingTest):
    ...
    def test_create_route(self):
        ...
        before_routes = self._check_table_state("joggingroute", "RouteName LIKE 'Test Route%'")
        ...
        after_routes = self._check_table_state("joggingroute", "RouteName LIKE 'Test Route%'")
        
        self.assertEqual(len(before_routes), 0, "There should be no test routes before the operation")
        self.assertEqual(len(after_routes), 1, "There should be one test route after the operation")
    ...
    def test_update_route(self):
        ...
        before_route = self._check_table_state("joggingroute", "RouteID = %s", (route_id,))[0]
        ...
        after_route = self._check_table_state("joggingroute", "RouteID = %s", (route_id,))[0]
    ...
    def test_delete_route(self):
        ...
        before_state = self._check_table_state("joggingroute", "RouteID = %s", (route_id,))
        
        with get_db_cursor(commit=False, connection=self.connection) as cursor:
            cursor.execute("DELETE FROM joggingroute WHERE RouteID = %s", (route_id,))
        
        after_state = self._check_table_state("joggingroute", "RouteID = %s", (route_id,))
