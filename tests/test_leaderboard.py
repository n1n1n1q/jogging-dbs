...
class LeaderboardTest(BaseJoggingTest):
    ...
    def test_leaderboard_generation(self):
        ...
        self.assertEqual(len(leaderboard), len(self.test_joggers), 
                         "Leaderboard should have an entry for each jogger")
        
        for i, entry in enumerate(leaderboard):
            expected_distance = 5.0 - (i * 0.5)
            expected_rank = i + 1
            self.assertEqual(entry["Rank"], expected_rank, 
                             f"Entry at position {i} should have rank {expected_rank}")
            self.assertEqual(entry["Distance"], expected_distance, 
        ...
        self.assertEqual(sorted(leaderboard_emails), sorted(jogger_emails), 
    ...
    def test_jogger_ranking(self):
        ...
        self.assertEqual(jogger_rank["Rank"], expected_rank, 
                         f"Jogger {target_jogger} should have rank {expected_rank}")
        self.assertEqual(jogger_rank["Distance"], 4.0,
    ...
    def test_top_performers_report(self):
        ...
        with get_db_cursor(connection=self.connection) as cursor:
            cursor.execute("""
                ...
                LIMIT 3
            """.format(",".join(["%s"] * len(event_ids))), tuple(event_ids))
            top_performers = cursor.fetchall()
        
        self.assertEqual(len(top_performers), min(3, len(self.test_joggers)), 
                         "Should have top 3 performers (or all joggers if fewer)")
        
        self.assertEqual(top_performers[0]["JoggerEmail"], self.test_joggers[0]["email"],
                         "First jogger should be the top performer")
        self.assertEqual(top_performers[0]["TotalDistance"], 10.0 * len(event_ids),
                         f"Top performer should have 10km x {len(event_ids)} events = {10.0 * len(event_ids)}km")
        self.assertEqual(top_performers[0]["EventCount"], len(event_ids),
